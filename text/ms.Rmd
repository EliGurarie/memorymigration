...
An illustration of the ability to adjust its migration to find a stable, migratory state that matches the dynamics of the resource is illustrated in Figure X.  Under many combinations of parameters, this kind of steady state adjustment is obtained and maintained.  

All variables contribute to the ability of the organism to find a stable, migratory state that matches the resource dynamics and maximizes foraging efficiency.  

In general, greater values of $\alpha$ (resource following) lead to an improved ability to match the migration,

Resource peaks with larger spatial extent (high $\sigma_x$) are smaller duration (low $\sigma_t$) are generally better for migration matching, suggesting that broad but short pulses have the strongest impact on the migration memory. 

Larger values of social attraction ($\beta$) generally lead to poorer migration matching except in those cases where the sociality scale $\lambda$ was high.  Thus, for example, at $\lambda = 20$, no simulations at $\beta >= 200$ managed to acquire or maintain a matched migration.  However, at $\lambda = 50$ or $100$, the migration was slightly better matched at high values of $\beta.  (Fig. **X**) 

Higher diffusion rates 

A random forest analysis indicated that the most important variables for attaining optimal migratory behavior were $\alpha$ and $\lambda$, and the least important, consistently, was $\sigma_t$ (table **X**). 
...
*Under what conditions can we adapt.*>
...
The model had, in general, a difficult time learning migration from a non-migratory initial condition.  Curiously, the conditions that were more conducive to learning migration were pulses of *longer* duration (high $\sigma_t$)
 but narrow spatial extent (low $\sigma_x$).  This was in contrast to the conditions that were most amenable for stable migratory behavior (high $\sigma_x$, intermediate $\sigma_t$).   Furthermore 

*Under what conditions can be learn.*
...
However, the ability to migrate was highly dependent on the nature of the resource dynamics.  In particular, the reinforcement of memory and foraging is strongest when patches are concentrated in time, but relatively large in space.  **ALSO SOME STUFF WE LEARN ABOUT THE INTERACTION BETWEEN DIFFUSION, RESOURCE FOLLOWING, SOCIALITY**
...
**Describing conditions for adaptability / stability.**
