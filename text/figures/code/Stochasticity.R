...
png("text/figures/TrendStochasticity_v2.png", width = 2400*2, height = 1000*2, res = 600)
{
...
plotFE <- function(df, ...){
  ...
  abline(h = c(0,1), lty = c(1,3), lwd = 3, col = "grey", xpd = FALSE)
  with(df, {
    points(kappa, FE_mean, col = cols[factor(beta_x)], pch = 19, ...)
    segments(kappa, FE_low, kappa, FE_high, col = cols[factor(beta_x)])
    lines(kappa[beta_x == -.25], FE_mean[beta_x == -.25], col = cols[2])
    lines(kappa[beta_x == -.5], FE_mean[beta_x == -.5], col = cols[1])
  })
  box(bty = "l")
}
...

{
  par(mfrow = c(1,4), mar = c(0,1,0,1), oma = c(4,4,4,4), tck = 0.01, mgp = c(1,.25,0), 
      bty = "l", yaxt = "n", xpd = NA)
  subset(cc.summary, psi_x == 0) %>% plotFE(cex = 1.5); axis(2, xpd = NA)
  mtext(side = 2, 
        at = seq(-.4,1.4,.2), 
        seq(-.4,1.4,.2), las = 1, line = .5, cex = 0.75)
  mtext(side = 2, 
  ...
  for(psi in c(3,6,12)){
    subset(cc.summary, psi_x == psi) %>% plotFE()
    mtext(bquote(psi[x] == .(psi)), side = 3, line = 1)
  }
  mtext(side = 1, outer = TRUE, expression("proportion reference memory"~(kappa)), line = 2.5, 
        cex = 1.1)
