...
fr1 <- dnorm(world$X.mid, mean = 80, sd = 10) %>% {./sum(.)/world$dx}
fr2 <- dnorm(world$X.mid, mean = 20, sd = 10) %>% {./sum(.)/world$dx}
...
with(world, 
  image(X.mid, 1:tau, resource, main = "resource", ylab = "x")
)
```
...
eval <- FALSE
if(eval){
  FixedResourceSim <- runManyYears(world, Parameters = parameters, n.years = 3)
  save(FixedResourceSim, file = "../results/FixedResourceSim.rda")
} else load("../results/FixedResourceSim.rda")
  

par(mfrow = c(1,2), oma = c(0,0,3,0))
plotYearList(FixedResourceSim, tau = tau)
plotYearList(FixedResourceSim, tau = tau, persp = TRUE, scale = FALSE, expand = 100)
title(printParameters(parameters), outer = TRUE)
```
...
world.R2 <- world %>% 
  list_modify(resource = getPulsedResource(1:world$tau, world$X.mid, c(t.peak = 25, t.sd = 9, x.peak = 80, x.sd = 5)))

world.R3 <- world %>% 
  list_modify(resource = getPulsedResource(1:world$tau, world$X.mid, c(t.peak = 25, t.sd = 5, x.peak = 80, x.sd = 9)))

world.R4 <- world %>% 
  list_modify(resource = getPulsedResource(1:world$tau, world$X.mid, c(t.peak = 25, t.sd = 5, x.peak = 80, x.sd = 5)))

par(mfrow = c(2,2), mar = c(3,3,3,1), mgp = c(2,.25,0), las = 1, tck = 0.01)
with(world.R1, {     
  image(1:tau,X.mid,resource, main = "Broad, long peak")
  contour(1:tau,X.mid,resource, add = TRUE)
  })
with(world.R2, {     
  image(1:tau,X.mid,resource, main = "Narrow, long peak")
  contour(1:tau,X.mid,resource, add = TRUE)
  })
with(world.R3, {     
  image(1:tau,X.mid,resource, main = "Broad, short peak")
  contour(1:tau,X.mid,resource, add = TRUE)
  })
with(world.R4, {     
  image(1:tau,X.mid,resource, main = "Narrow, short peak")
  contour(1:tau,X.mid,resource, add = TRUE)
  })
...
parameters <- c(epsilon = 5, alpha = 0, beta0 = 20, beta1 = 5e2)
Memory_PulsedResource_Sim <- runManyYears(world.R3, Parameters = parameters, n.years = 6)
...
Tactic_PulsedResource_Sim <- runManyYears(world.R1, Parameters = parameters, n.years = 6)
...
Hybrid_PulsedResource_Sim <- runManyYears(world.R1, Parameters = parameters, n.years = 6)
...
Tactic_PulsedResource_Sim2 <- runManyYears(world.R1, Parameters = parameters, n.years = 6)
...
parameters <- c(epsilon = 5, alpha = 2e2, beta0 = 20, beta1 = 3e2)
Hybrid_PulsedResource_Sim3 <- runManyYears(world.R4, Parameters = parameters, n.years = 6)

par(mfrow = c(1,2), oma = c(0,0,3,0))
plotYearList(Hybrid_PulsedResource_Sim3, tau = tau)
plotYearList(Hybrid_PulsedResource_Sim3, tau = tau, persp = TRUE, scale = FALSE, expand = 100)
title(printParameters(parameters), outer = TRUE)
```
